...
func init() {
	...
	if secret == "" {
		...
		if data, err := os.ReadFile("data/hmac_secret.key"); err == nil {
		...
		if secret == "" {
			...
			os.WriteFile("data/hmac_secret.key", []byte(secret), 0600)
			log.Println("[AUTH] Generated new HMAC secret (saved to data/hmac_secret.key)")
		}
	}
