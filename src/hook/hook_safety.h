...
static inline size_t CefClientHandlerTableOffset(void) {
    return offsetof(cef_client_t, get_audio_handler);
}

static inline bool IsInvalidCodePointer(PVOID value) {
    UINT_PTR raw = (UINT_PTR)value;
    return raw == 0 || raw == (UINT_PTR)-1;
}

static inline bool DllPlaintextLengthFromEncryptedSize(DWORD encryptedLen, DWORD* plainLen) {
